...
gentity_t *G_TestEntityPosition(gentity_t *ent)
{
    ...
    if (ent->client)
        G_Trace(&tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs,
                ent->client->ps.origin, ent->s.number, ent->clipmask);
    else
        G_Trace(&tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs,
                ent->s.pos.trBase, ent->s.number, ent->clipmask);
...
void Think_ClosedModelDoor(gentity_t *ent)
{
    ...
    if (ent->teammaster == ent || !ent->teammaster)
        G_AdjustAreaPortalState(ent, false);
...
void SP_func_door_model(gentity_t *ent)
{
    ...
    G_SetBrushModel(clipBrush, clipBrush->model);
    clipBrush->s.eType = ET_INVISIBLE;
    G_LinkEntity(clipBrush);
