...
void G_RestoreCvars(void)
{
    ...
    for (i = 0, cv = gameCvarTable; i < gameCvarTableSize; i++, cv++)
    {
        if (cv->vmCvar && cv->_explicit)
            Cvar_Set(cv->cvarName, cv->_explicit);
    }
}
...
void G_MapConfigs(const char *mapname)
{
    ...
    Cbuf_ExecuteText(EXEC_APPEND, va("exec \"%s/default.cfg\"\n", g_mapConfigs.string));

    Cbuf_ExecuteText(EXEC_APPEND, va("exec \"%s/%s.cfg\"\n", g_mapConfigs.string, mapname));

    Cvar_Set("g_mapConfigsLoaded", "1");
}
...
void G_InitGame(int levelTime, int randomSeed, int restart)
{
    ...
    SV_LocateGameData( static_cast<sharedEntity_t*>((void*)&level.gentities[0]),
    ...
    // reset stages
    Cvar_Set("g_alienStage", va("%d", S1));
    Cvar_Set("g_humanStage", va("%d", S1));
    Cvar_Set("g_alienCredits", 0);
    Cvar_Set("g_humanCredits", 0);
...
void G_CalculateBuildPoints(void)
{
    ...
    if (G_TimeTilSuddenDeath() <= 0 && level.suddenDeathWarning < TW_PASSED)
    {
        ...
        SV_GameSendServerCommand(-1, "cp \"Sudden Death!\"");
    ...
    {
        SV_GameSendServerCommand(-1, va("cp \"Sudden Death in %d seconds!\"", (int)(G_TimeTilSuddenDeath() / 1000)));
        SV_GameSendServerCommand(
...
void CheckExitRules(void)
{
    ...
    if (g_timelimit.integer)
    {
        ...
        {
            SV_GameSendServerCommand(-1, "cp \"5 minutes remaining!\"");
            level.timelimitWarning = TW_IMMINENT;
        }
        else if (level.time - level.startTime >= (g_timelimit.integer - 1) * 60000 &&
                 level.timelimitWarning < TW_PASSED)
        {
            SV_GameSendServerCommand(-1, "cp \"1 minute remaining!\"");
            level.timelimitWarning = TW_PASSED;
        }
    }
...
void G_Vote(gentity_t *ent, team_t team, bool voting)
{
    ...
    if (ent->client->pers.vote & (1 << team))
    {
        ...
        SV_SetConfigstring(CS_VOTE_YES + team, va("%d", level.voteYes[team]));
    }
    else
    {
        if (voting)
        ...
        SV_SetConfigstring(CS_VOTE_NO + team, va("%d", level.voteNo[team]));
    }
}
...
void G_CheckVote(team_t team)
{
    ...
    SV_SetConfigstring(CS_VOTE_YES + team, "0");
    SV_SetConfigstring(CS_VOTE_NO + team, "0");
}
