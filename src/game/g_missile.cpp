...
void G_RunMissile(gentity_t *ent)
{
    ...
    if (tr.fraction < 1.0f)
    {
        ...
        {
            G_Trace(&tr, ent->r.currentOrigin, NULL, NULL, origin, passent, ent->clipmask);
            ...
            else
            {
                ...
                {
                    G_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, CONTENTS_BODY);
    ...
    ent->r.contents = CONTENTS_SOLID;  // trick G_LinkEntity into...
    G_LinkEntity(ent);
...
void AHive_SearchAndDestroy(gentity_t *self)
{
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        ...
        {
            G_Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs, ent->r.currentOrigin, self->r.ownerNum,
                self->clipmask);
