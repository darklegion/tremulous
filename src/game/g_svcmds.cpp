...
static void Svcmd_CenterPrint_f(void)
{
    ...
    SV_GameSendServerCommand(-1, va("cp \"%s\"", ConcatArgs(1)));
}
...
static void Svcmd_SuddenDeath_f(void)
{
    ...
    SV_GameSendServerCommand(-1, va("cp \"Sudden Death will begin in %d second%s\"", offset, offset == 1 ? "" : "s"));
}
...
void G_RegisterCommands(void)
{
    ...
    for (i = 0; i < ARRAY_LEN(svcmds); i++)
    {
        if (svcmds[i].dedicated && !g_dedicated.integer)
            continue;
        Cmd_AddCommand(svcmds[i].cmd, nullptr);
    }
