...
===============
*/
bool G_MapExists(const char *name) { return FS_FOpenFileByMode(va("maps/%s.bsp", name), NULL, FS_READ) > 0; }
...
bool G_LayoutExists(const char *map, const char *layout)
{
    return !Q_stricmp(layout, "*BUILTIN*") ||
           FS_FOpenFileByMode(va("layouts/%s/%s.dat", map, layout), NULL, FS_READ) > 0;
}
...
static void G_PushRotationStack(int rotation)
{
    ...
    Cvar_Set("g_mapRotationStack", text);
    Cvar_Update(&g_mapRotationStack);
}
...
static int G_PopRotationStack(void)
{
    ...
    if (text_p)
    {
        while (*text_p == ' ')
            text_p++;
        Cvar_Set("g_mapRotationStack", text_p);
        Cvar_Update(&g_mapRotationStack);
    }
    else
        G_ClearRotationStack();
...
static void G_SetCurrentNodeByIndex(int currentNode, int rotation)
{
    ...
    Cvar_Set("g_mapRotationNodes", text);
...
static void G_IssueMapChange(int index, int rotation)
{
    ...
    {
        Cvar_Set("g_nextLayout", map->layouts);
    }
...
bool G_StartMapRotation(char *name, bool advance, bool putOnStack, bool reset_index, int depth)
{
    ...
    for (i = 0; i < mapRotations.numRotations; i++)
    {
        if (!Q_stricmp(mapRotations.rotations[i].name, name))
        {
            ...
            Cvar_Set("g_currentMapRotation", va("%d", i));
...
{
    Cvar_Set("g_currentMapRotation", va("%d", NOT_ROTATING));
    Cvar_Update(&g_currentMapRotation);
}
