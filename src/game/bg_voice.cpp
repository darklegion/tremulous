...
static voice_t *BG_VoiceList(void)
{
    ...
    numFiles = FS_GetFileList("voice", ".voice", fileList, sizeof(fileList));
...
static voiceTrack_t *BG_VoiceParseCommand(int handle)
{
    ...
    while (Parse_ReadTokenHandle(handle, &token))
    {
        ...
        {
#ifdef CGAME
            voiceTracks->track = S_RegisterSound(token.string, false);
            voiceTracks->duration = S_SoundDuration(voiceTracks->track);
#endif
        }
