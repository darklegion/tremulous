    ...
    int spawnNum, const vec3_t origin, const vec3_t normal, buildable_t spawn, vec3_t spawnOrigin)
{
    ...
    G_Trace(&tr, origin, NULL, NULL, localOrigin, spawnNum, MASK_SHOT);

    if (tr.entityNum != ENTITYNUM_NONE)
        return &g_entities[tr.entityNum];

    G_Trace(&tr, localOrigin, cmins, cmaxs, localOrigin, ENTITYNUM_NONE, MASK_PLAYERSOLID);
...
static void G_CreepSlow(gentity_t *self)
{
    ...
    // find humans
    int num = SV_AreaEntities(mins, maxs, entityList, MAX_GENTITIES);
    for (int i = 0; i < num; i++)
    {
...
void AGeneric_Blast(gentity_t *self)
{
    ...
    G_LinkEntity(self);  //...requires a relink
}
...
void ABarricade_Shrink(gentity_t *self, bool shrink)
{
    ...
    else
    {
        ...
        G_Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs, self->r.currentOrigin, self->s.number,
            MASK_PLAYERSOLID);
...
static bool AHive_CheckTarget(gentity_t *self, gentity_t *enemy)
{
    ...
    G_Trace(&trace, tip_origin, NULL, NULL, enemy->s.pos.trBase, self->s.number, MASK_SHOT);
...
bool ATrapper_CheckTarget(gentity_t *self, gentity_t *target, int range)
{
    ...
    G_Trace(&trace, self->s.pos.trBase, NULL, NULL, target->s.pos.trBase, self->s.number, MASK_SHOT);
...
bool HMGTurret_CheckTarget(gentity_t *self, gentity_t *target, bool los_check)
{
    ...
    G_Trace(&tr, self->s.pos.trBase, NULL, NULL, end, self->s.number, MASK_SHOT);
...
void G_BuildableTouchTriggers(gentity_t *ent)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        if (!G_EntityContact(mins, maxs, hit, TT_AABB))
            continue;
    ...
    gentity_t *ent, buildable_t buildable, int distance, vec3_t origin, vec3_t normal, int *groundEntNum)
{
    ...
    BG_PositionBuildableRelativeToPlayer(ps, mins, maxs, G_Trace, entity_origin, angles, &tr1);
...
static gentity_t *G_FinishSpawningBuildable(gentity_t *ent, bool force)
{
    ...
    G_Trace(&tr, built->r.currentOrigin, built->r.mins, built->r.maxs, dest, built->s.number, built->clipmask);
...
int G_LayoutList(const char *map, char *list, int len)
{
    ...
    numFiles = FS_GetFileList(va("layouts/%s", map), ".dat", fileList, sizeof(fileList));
    filePtr = fileList;
...
loadAnotherLayout:
    ...
    Cvar_VariableStringBuffer("mapname", map, sizeof(map));
    len = FS_FOpenFileByMode(va("layouts/%s/%s.dat", map, lstr), &f, FS_READ);
    if (len < 0)
    {
...
void G_UpdateBuildableRangeMarkers(void)
{
    ...
    bool maskingExtension = (Cvar_VariableIntegerValue("sv_gppExtension") >= 1);
        ...
        for (int i = 0; i < level.maxclients; ++i)
        {
            ...
            if (i >= 32 && !maskingExtension)
            {
                ...
                if (!Cvar_VariableIntegerValue("g_rangeMarkerWarningGiven"))
                {
                    ...
                    Cvar_Set("g_rangeMarkerWarningGiven", "1");
                }
