...
gentity_t *G_Spawn(void)
{
    ...
    SV_LocateGameData( static_cast<sharedEntity_t*>((void*)&level.gentities[0]),
        level.num_entities, sizeof(gentity_t), &level.clients[0].ps, sizeof(level.clients[0]));
...
{
    G_UnlinkEntity(ent);  // unlink from world
...
void G_RemoveEntity(gentity_t *ent)
{
    ...
    else if (ent->s.eType == ET_MOVER)
    {
        ...
        G_AdjustAreaPortalState(ent, true);
    }
...
bool G_Visible(gentity_t *ent1, gentity_t *ent2, int contents)
{
    ...
    G_Trace(&trace, ent1->s.pos.trBase, NULL, NULL, ent2->s.pos.trBase, ent1->s.number, contents);
...
void G_TraceCapsule( trace_t *results, const vec3_t start,
                            ...
                            int contentMask )
{
    SV_Trace( results, start, mins, maxs, end, passEntityNum, contentMask, TT_CAPSULE );
}

void G_LinkEntity(gentity_t *gEnt)
{
    SV_LinkEntity(static_cast<sharedEntity_t*>((void*)gEnt));
}

void G_UnlinkEntity(gentity_t *gEnt)
{
    SV_UnlinkEntity(static_cast<sharedEntity_t*>((void*)gEnt));
}

void G_AdjustAreaPortalState(gentity_t *ent, bool open)
{
    SV_AdjustAreaPortalState(static_cast<sharedEntity_t*>((void*)ent), open);
}

bool G_EntityContact( vec3_t mins, vec3_t maxs, const gentity_t *gEnt, traceType_t type )
{
    return SV_EntityContact( mins, maxs, static_cast<const sharedEntity_t*>((void*)gEnt), type );
}

void G_SetBrushModel( gentity_t *ent, const char *name )
{
    SV_SetBrushModel(static_cast<sharedEntity_t*>((void*)ent), name);
}
