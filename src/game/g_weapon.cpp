...
static void G_WideTrace(trace_t *tr, gentity_t *ent, float range, float width, float height, gentity_t **target)
{
    ...
    *target = NULL;
    ...
    G_Trace(tr, muzzle, mins, maxs, end, ent->s.number, CONTENTS_BODY);
    ...
    G_Trace(tr, muzzle, NULL, NULL, end, ent->s.number, CONTENTS_SOLID);
    if (tr->entityNum != ENTITYNUM_NONE)
        *target = &g_entities[tr->entityNum];
...
void bulletFire(gentity_t *ent, float spread, int damage, int mod)
{
    ...
    if (ent->client)
    {
        ...
        G_Trace(&tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT);
        G_UnlaggedOff();
    }
    else
        G_Trace(&tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT);
...
void ShotgunPattern(vec3_t origin, vec3_t origin2, int seed, gentity_t *ent)
{
    ...
    for (i = 0; i < SHOTGUN_PELLETS; i++)
    {
        ...
        G_Trace(&tr, origin, NULL, NULL, end, ent->s.number, MASK_SHOT);
        traceEnt = &g_entities[tr.entityNum];
...
void teslaFire(gentity_t *self)
{
    ...
    G_Trace(&tr, origin, NULL, NULL, target, self->s.number, MASK_SHOT);
...
void poisonCloud(gentity_t *ent)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        {
            G_Trace(&tr, muzzle, NULL, NULL, humanPlayer->r.currentOrigin, humanPlayer->s.number, CONTENTS_SOLID);
            ...
            SV_GameSendServerCommand(humanPlayer->client->ps.clientNum, "poisoncloud");
        }
    }
    G_UnlaggedOff();
}
...
static void G_FindZapChainTargets(zap_t *zap)
{
    ...
    for (i = 0; i < num; i++)
    {
            ...
            distance <= LEVEL2_AREAZAP_CHAIN_RANGE)
        {
            ...
            G_Trace(&tr, ent->r.currentOrigin, NULL, NULL, enemy->r.currentOrigin, ent->s.number, CONTENTS_SOLID);
...
static void G_UpdateZapEffect(zap_t *zap)
{
    ...
    G_LinkEntity(zap->effectChannel);
}
