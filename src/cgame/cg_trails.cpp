...
static void CG_LightVertex(vec3_t point, byte alpha, byte *rgba)
{
    ...
    re->LightForPoint(point, alight, dlight, lightdir);
    for (i = 0; i <= 2; i++)
        rgba[i] = (int)alight[i];
...
static void CG_RenderBeam(trailBeam_t *tb)
{
    ...
    re->AddPolyToScene(tb->klass->shader, 4, &verts[0], numVerts / 4);
}
...
void CG_LoadTrailSystems(void)
{
    ...
    numFiles = FS_GetFileList("scripts", ".trail", fileList, MAX_TRAIL_FILES * MAX_QPATH);
    filePtr = fileList;
    ...
    for (i = 0; i < MAX_BASETRAIL_SYSTEMS; i++)
    {
        ...
        if (!Q_stricmp(bts->name, name))
        {
            ...
            for (j = 0; j < bts->numBeams; j++)
            {
                ...
                btb->shader = re->RegisterShader(btb->shaderName);
            }
