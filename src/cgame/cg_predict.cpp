    ...
    int skipNumber, int mask, trace_t *tr, traceType_t collisionType)
{
    ...
    for (i = 0; i < j; i++)
    {
        ...
        if (collisionType == TT_CAPSULE || collisionType == TT_AABB )
        {
            CM_TransformedBoxTrace(&trace, start, end, mins, maxs, cmodel, mask, origin, angles, collisionType);
        }
        else if (collisionType == TT_BISPHERE)
        {
            CM_TransformedBiSphereTrace(&trace, start, end, mins[0], maxs[0], cmodel, mask, origin);
        }
        ...
        const vec3_t end, int skipNumber, int mask)
{
    ...
    CM_BoxTrace(&t, start, end, mins, maxs, 0, mask, TT_AABB);
...
================
*/
void CG_CapTrace(trace_t *result, const vec3_t start, vec3_t mins, vec3_t maxs, const vec3_t end,
    int skipNumber, int mask)
{
    trace_t t;

    CM_BoxTrace(&t, start, end, mins, maxs, 0, mask, TT_CAPSULE);
    t.entityNum = t.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
    // check all other solid models
    ...
    const float endRadius, int skipNumber, int mask)
{
    ...
    CM_BiSphereTrace(&t, start, end, startRadius, endRadius, 0, mask);
    t.entityNum = t.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
    // check all other solid models
...
static void CG_TouchTriggerPrediction(void)
{
    ...
    for (i = 0; i < cg_numTriggerEntities; i++)
    {
        ...
        CM_BoxTrace(&trace, cg.predictedPlayerState.origin, cg.predictedPlayerState.origin, cg_pmove.mins,
            cg_pmove.maxs, cmodel, -1, TT_AABB);
