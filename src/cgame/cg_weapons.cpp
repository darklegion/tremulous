...
void CG_RegisterUpgrade(int upgradeNum)
{
    ...
    if (upgradeNum == UP_GRENADE)
        ...
        upgradeInfo->upgradeIcon = re->RegisterShader(icon);
}
...
static bool CG_ParseWeaponModeSection(weaponInfoMode_t *wim, char **text_p)
{
    ...
    while (1)
    {
        ...
        if (!Q_stricmp(token, "missileModel"))
        {
            ...
            wim->missileModel = re->RegisterModel(token);
        ...
        else if (!Q_stricmp(token, "missileSprite"))
        {
            ...
            wim->missileSprite = re->RegisterShader(token);
        ...
        else if (!Q_stricmp(token, "impactMark"))
        {
            ...
            wim->impactMark = re->RegisterShader(token);
        ...
        else if (!Q_stricmp(token, "impactSound"))
        {
            ...
            wim->impactSound[index] = S_RegisterSound(token, false);
        ...
        else if (!Q_stricmp(token, "impactFleshSound"))
        {
            ...
            wim->impactFleshSound[index] = S_RegisterSound(token, false);
        ...
        else if (!Q_stricmp(token, "firingSound"))
        {
            ...
            wim->firingSound = S_RegisterSound(token, false);
        ...
        else if (!Q_stricmp(token, "missileSound"))
        {
            ...
            wim->missileSound = S_RegisterSound(token, false);
        ...
        else if (!Q_stricmp(token, "flashSound"))
        {
            ...
            wim->flashSound[index] = S_RegisterSound(token, false);
...
static bool CG_ParseWeaponFile(const char *filename, weaponInfo_t *wi)
{
    ...
    while (1)
    {
        ...
        else if (!Q_stricmp(token, "weaponModel"))
        {
            ...
            wi->weaponModel = re->RegisterModel(token);
            ...
            wi->flashModel = re->RegisterModel(path);

            strcpy(path, token);
            COM_StripExtension(path, path, MAX_QPATH);
            strcat(path, "_barrel.md3");
            wi->barrelModel = re->RegisterModel(path);

            strcpy(path, token);
            COM_StripExtension(path, path, MAX_QPATH);
            strcat(path, "_hand.md3");
            wi->handsModel = re->RegisterModel(path);
        ...
        else if (!Q_stricmp(token, "weaponModel3rdPerson"))
        {
            ...
            wi->weaponModel3rdPerson = re->RegisterModel(token);
            ...
            COM_StripExtension(path, path, MAX_QPATH);
            strcat(path, "_flash.md3");
            wi->flashModel3rdPerson = re->RegisterModel(path);

            strcpy(path, token);
            COM_StripExtension(path, path, MAX_QPATH);
            strcat(path, "_barrel.md3");
            wi->barrelModel3rdPerson = re->RegisterModel(path);
        ...
        else if (!Q_stricmp(token, "idleSound"))
        {
            ...
            wi->readySound = S_RegisterSound(token, false);
        ...
        else if (!Q_stricmp(token, "icon"))
        {
            ...
            wi->weaponIcon = wi->ammoIcon = re->RegisterShader(token);
        ...
        else if (!Q_stricmp(token, "crosshair"))
        {
            ...
            wi->crossHair = re->RegisterShader(token);
...
void CG_RegisterWeapon(int weaponNum)
{
    ...
    re->ModelBounds(weaponInfo->weaponModel, mins, maxs);
    for (i = 0; i < 3; i++)
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
    ...
    if (ps || cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum)
    {
        ...
        {
            re->AddLightToScene(flash.origin, 300 + (rand() & 31), weapon->wim[weaponMode].flashDlightColor[0],
...
void CG_NextWeapon_f(void)
{
    ...
    if (cg.snap->ps.pm_flags & PMF_FOLLOW)
    {
        CL_AddReliableCommand("followprev\n", false);
        return;
    }
...
void CG_PrevWeapon_f(void)
{
    ...
    if (cg.snap->ps.pm_flags & PMF_FOLLOW)
    {
        CL_AddReliableCommand("follownext\n", false);
        return;
    }
...
void CG_FireWeapon(centity_t *cent, weaponMode_t weaponMode)
{
    ...
    if (c > 0)
    {
        c = rand() % c;
        if (wi->wim[weaponMode].flashSound[c])
            S_StartSound(NULL, es->number, CHAN_WEAPON, wi->wim[weaponMode].flashSound[c]);
    }
}
    ...
    impactSound_t soundType, int charge)
{
    ...
    if (soundType == IMPACTSOUND_FLESH)
    {
        ...
        if (c > 0)
        {
            c = rand() % c;
            if (weapon->wim[weaponMode].impactFleshSound[c])
                S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO, weapon->wim[weaponMode].impactFleshSound[c]);
        }
    }
    else
    {
        ...
        if (c > 0)
        {
            c = rand() % c;
            if (weapon->wim[weaponMode].impactSound[c])
                S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO, weapon->wim[weaponMode].impactSound[c]);
        }
    }
...
void CG_Tracer(vec3_t source, vec3_t dest)
{
    ...
    re->AddPolyToScene(cgs.media.tracerShader, 4, verts, 1);
