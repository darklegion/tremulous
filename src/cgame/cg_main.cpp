...
static void CG_SetUIVars(void)
{
    ...
    *carriageCvar = 0;
    ...
    Cvar_Set("ui_carriage", carriageCvar);

    Cvar_Set("ui_stages", va("%d %d", cgs.alienStage, cgs.humanStage));
}
...
static void CG_SetPVars(void)
{
    ...
    Cvar_Set("player_hp", va("%d", ps->stats[STAT_HEALTH]));
    Cvar_Set("player_maxhp", va("%d", ps->stats[STAT_MAX_HEALTH]));
    ...
    switch (ps->stats[STAT_TEAM])
    {
        case TEAM_NONE:
            Cvar_Set("team_bp", "0");
            Cvar_Set("team_kns", "0");
            Cvar_Set("team_teamname", "spectator");
            Cvar_Set("team_stage", "0");
            break;

        case TEAM_ALIENS:
            // Cvar_Set( "team_bp", va( "%d", cgs.alienBuildPoints ));
            Cvar_Set("team_kns", va("%d", cgs.alienNextStageThreshold));
            Cvar_Set("team_teamname", "aliens");
            Cvar_Set("team_stage", va("%d", cgs.alienStage + 1));
            break;

        case TEAM_HUMANS:
            // Cvar_Set( "team_bp", va("%d",cgs.humanBuildPoints) );
            Cvar_Set("team_kns", va("%d", cgs.humanNextStageThreshold));
            Cvar_Set("team_teamname", "humans");
            Cvar_Set("team_stage", va("%d", cgs.humanStage + 1));
            break;
    }

    Cvar_Set("player_credits", va("%d", cg.snap->ps.persistant[PERS_CREDIT]));
    Cvar_Set("player_score", va("%d", cg.snap->ps.persistant[PERS_SCORE]));

    if (CG_LastAttacker() != -1)
        Cvar_Set("player_attackername", cgs.clientinfo[CG_LastAttacker()].name);
    else
        Cvar_Set("player_attackername", "");

    if (CG_CrosshairPlayer() != -1)
        Cvar_Set("player_crosshairname", cgs.clientinfo[CG_CrosshairPlayer()].name);
    else
        Cvar_Set("player_crosshairname", "");
}
...
void CG_AddNotifyText(void)
{
    ...
    Cmd_LiteralArgsBuffer(buffer, BIG_INFO_STRING);
...
=================
*/
int CG_FileExists(const char *filename) { return FS_FOpenFileByMode(filename, NULL, FS_READ); }
...
static void CG_RegisterSounds(void)
{
    ...
    cgs.media.alienStageTransition = S_RegisterSound("sound/announcements/overmindevolved.wav", true);
    cgs.media.humanStageTransition = S_RegisterSound("sound/announcements/reinforcement.wav", true);

    cgs.media.alienOvermindAttack = S_RegisterSound("sound/announcements/overmindattack.wav", true);
    cgs.media.alienOvermindDying = S_RegisterSound("sound/announcements/overminddying.wav", true);
    cgs.media.alienOvermindSpawns = S_RegisterSound("sound/announcements/overmindspawns.wav", true);

    cgs.media.alienL1Grab = S_RegisterSound("sound/player/level1/grab.wav", true);
    cgs.media.alienL4ChargePrepare = S_RegisterSound("sound/player/level4/charge_prepare.wav", true);
    cgs.media.alienL4ChargeStart = S_RegisterSound("sound/player/level4/charge_start.wav", true);

    cgs.media.tracerSound = S_RegisterSound("sound/weapons/tracer.wav", false);
    cgs.media.selectSound = S_RegisterSound("sound/weapons/change.wav", false);
    cgs.media.turretSpinupSound = S_RegisterSound("sound/buildables/mgturret/spinup.wav", false);
    cgs.media.weaponEmptyClick = S_RegisterSound("sound/weapons/click.wav", false);

    cgs.media.talkSound = S_RegisterSound("sound/misc/talk.wav", false);
    cgs.media.alienTalkSound = S_RegisterSound("sound/misc/alien_talk.wav", false);
    cgs.media.humanTalkSound = S_RegisterSound("sound/misc/human_talk.wav", false);
    ...
    cgs.media.disconnectSound = S_RegisterSound("sound/misc/disconnect.wav", false);
    ...
    cgs.media.jetpackDescendSound = S_RegisterSound("sound/upgrades/jetpack/low.wav", false);
    cgs.media.jetpackIdleSound = S_RegisterSound("sound/upgrades/jetpack/idle.wav", false);
    cgs.media.jetpackAscendSound = S_RegisterSound("sound/upgrades/jetpack/hi.wav", false);

    cgs.media.medkitUseSound = S_RegisterSound("sound/upgrades/medkit/medkit.wav", false);

    cgs.media.alienEvolveSound = S_RegisterSound("sound/player/alienevolve.wav", false);

    cgs.media.alienBuildableExplosion = S_RegisterSound("sound/buildables/alien/explosion.wav", false);
    cgs.media.alienBuildableDamage = S_RegisterSound("sound/buildables/alien/damage.wav", false);
    cgs.media.alienBuildablePrebuild = S_RegisterSound("sound/buildables/alien/prebuild.wav", false);

    cgs.media.humanBuildableExplosion = S_RegisterSound("sound/buildables/human/explosion.wav", false);
    cgs.media.humanBuildablePrebuild = S_RegisterSound("sound/buildables/human/prebuild.wav", false);

    for (i = 0; i < 4; i++)
        cgs.media.humanBuildableDamage[i] = S_RegisterSound(va("sound/buildables/human/damage%d.wav", i), false);

    cgs.media.hardBounceSound1 = S_RegisterSound("sound/misc/hard_bounce1.wav", false);
    cgs.media.hardBounceSound2 = S_RegisterSound("sound/misc/hard_bounce2.wav", false);

    cgs.media.repeaterUseSound = S_RegisterSound("sound/buildables/repeater/use.wav", false);

    cgs.media.buildableRepairSound = S_RegisterSound("sound/buildables/human/repair.wav", false);
    cgs.media.buildableRepairedSound = S_RegisterSound("sound/buildables/human/repaired.wav", false);

    cgs.media.lCannonWarningSound = S_RegisterSound("models/weapons/lcannon/warning.wav", false);
    cgs.media.lCannonWarningSound2 = S_RegisterSound("models/weapons/lcannon/warning2.wav", false);
}
...
static void CG_RegisterGraphics(void)
{
    ...
    re->LoadWorld(cgs.mapname);
    CG_UpdateMediaFraction(0.66f);

    for (i = 0; i < 11; i++)
        cgs.media.numberShaders[i] = re->RegisterShader(sb_nums[i]);

    cgs.media.viewBloodShader = re->RegisterShader("gfx/damage/fullscreen_painblend");

    cgs.media.connectionShader = re->RegisterShader("gfx/2d/net");

    cgs.media.creepShader = re->RegisterShader("creep");

    cgs.media.scannerBlipShader = re->RegisterShader("gfx/2d/blip");
    cgs.media.scannerLineShader = re->RegisterShader("gfx/2d/stalk");

    cgs.media.teamOverlayShader = re->RegisterShader("gfx/2d/teamoverlay");

    cgs.media.tracerShader = re->RegisterShader("gfx/misc/tracer");

    cgs.media.backTileShader = re->RegisterShader("console");

    // building shaders
    cgs.media.greenBuildShader = re->RegisterShader("gfx/misc/greenbuild");
    cgs.media.redBuildShader = re->RegisterShader("gfx/misc/redbuild");
    cgs.media.humanSpawningShader = re->RegisterShader("models/buildables/telenode/rep_cyl");

    for (i = 0; i < 8; i++)
        cgs.media.buildWeaponTimerPie[i] = re->RegisterShader(buildWeaponTimerPieShaders[i]);

    // player health cross shaders
    cgs.media.healthCross = re->RegisterShader("ui/assets/neutral/cross.tga");
    cgs.media.healthCross2X = re->RegisterShader("ui/assets/neutral/cross2.tga");
    cgs.media.healthCross3X = re->RegisterShader("ui/assets/neutral/cross3.tga");
    cgs.media.healthCrossMedkit = re->RegisterShader("ui/assets/neutral/cross_medkit.tga");
    cgs.media.healthCrossPoisoned = re->RegisterShader("ui/assets/neutral/cross_poison.tga");

    cgs.media.upgradeClassIconShader = re->RegisterShader("icons/icona_upgrade.tga");

    cgs.media.balloonShader = re->RegisterShader("gfx/sprites/chatballoon");
    ...
    cgs.media.shadowMarkShader = re->RegisterShader("gfx/marks/shadow");
    cgs.media.wakeMarkShader = re->RegisterShader("gfx/marks/wake");
    ...
    cgs.media.sphereModel = re->RegisterModel("models/generic/sphere");
    cgs.media.sphericalCone64Model = re->RegisterModel("models/generic/sphericalCone64");
    cgs.media.sphericalCone240Model = re->RegisterModel("models/generic/sphericalCone240");

    cgs.media.plainColorShader = re->RegisterShader("gfx/plainColor");
    cgs.media.binaryAlpha1Shader = re->RegisterShader("gfx/binary/alpha1");

    for (i = 0; i < NUM_BINARY_SHADERS; ++i)
    {
        cgs.media.binaryShaders[i].f1 = re->RegisterShader(va("gfx/binary/%03i_F1", i));
        cgs.media.binaryShaders[i].f2 = re->RegisterShader(va("gfx/binary/%03i_F2", i));
        cgs.media.binaryShaders[i].f3 = re->RegisterShader(va("gfx/binary/%03i_F3", i));
        cgs.media.binaryShaders[i].b1 = re->RegisterShader(va("gfx/binary/%03i_B1", i));
        cgs.media.binaryShaders[i].b2 = re->RegisterShader(va("gfx/binary/%03i_B2", i));
        cgs.media.binaryShaders[i].b3 = re->RegisterShader(va("gfx/binary/%03i_B3", i));
    }
    ...
    for (i = 1; i < cgs.numInlineModels; i++)
    {
        ...
        cgs.inlineDrawModel[i] = re->RegisterModel(name);
        re->ModelBounds(cgs.inlineDrawModel[i], mins, maxs);
    ...
    for (i = 1; i < MAX_MODELS; i++)
    {
        ...
        cgs.gameModels[i] = re->RegisterModel(modelName);
    }
    ...
    for (i = 1; i < MAX_GAME_SHADERS; i++)
    {
        ...
        cgs.gameShaders[i] = re->RegisterShader(shaderName);
    }
...
static void CG_RegisterClients(void)
{
    ...
    cgs.media.larmourHeadSkin = re->RegisterSkin("models/players/human_base/head_light.skin");
    cgs.media.larmourLegsSkin = re->RegisterSkin("models/players/human_base/lower_light.skin");
    cgs.media.larmourTorsoSkin = re->RegisterSkin("models/players/human_base/upper_light.skin");

    cgs.media.jetpackModel = re->RegisterModel("models/players/human_base/jetpack.md3");
    cgs.media.jetpackFlashModel = re->RegisterModel("models/players/human_base/jetpack_flash.md3");
    cgs.media.battpackModel = re->RegisterModel("models/players/human_base/battpack.md3");
...
bool CG_Asset_Parse(int handle)
{
    ...
    while (1)
    {
        ...
        if (Q_stricmp(token.string, "gradientbar") == 0)
        {
            ...
            cgDC.Assets.gradientBar = re->RegisterShaderNoMip(tempStr);
            continue;
        }
        ...
        if (Q_stricmp(token.string, "menuEnterSound") == 0)
        {
            ...
            cgDC.Assets.menuEnterSound = S_RegisterSound(tempStr, false);
            continue;
        }
        ...
        if (Q_stricmp(token.string, "menuExitSound") == 0)
        {
            ...
            cgDC.Assets.menuExitSound = S_RegisterSound(tempStr, false);
            continue;
        }
        ...
        if (Q_stricmp(token.string, "itemFocusSound") == 0)
        {
            ...
            cgDC.Assets.itemFocusSound = S_RegisterSound(tempStr, false);
            continue;
        }
        ...
        if (Q_stricmp(token.string, "menuBuzzSound") == 0)
        {
            ...
            cgDC.Assets.menuBuzzSound = S_RegisterSound(tempStr, false);
            continue;
        }
        ...
        if (Q_stricmp(token.string, "cursor") == 0)
        {
            ...
            cgDC.Assets.cursor = re->RegisterShaderNoMip(cgDC.Assets.cursorStr);
            continue;
        }
...
void CG_ParseMenu(const char *menuFile)
{
    ...
    if (!handle)
        handle = Parse_LoadSourceHandle("ui/testhud.menu");
...
void CG_LoadMenus(const char *menuFile)
{
    ...
    if (!f)
    {
        Com_Error(CG_ERROR, S_COLOR_YELLOW "menu file not found: %s, using default", menuFile);
        len = FS_FOpenFileByMode("ui/hud.txt", &f, FS_READ);

        if (!f)
            Com_Error(CG_ERROR, S_COLOR_RED "default menu file not found: ui/hud.txt, unable to continue!");
    }

    if (len >= MAX_MENUDEFFILE)
    {
        Com_Error(CG_ERROR, S_COLOR_RED "menu file too large: %s is %i, max allowed is %i",
...
static void CG_StopCinematic(int handle) { CIN_StopCinematic(handle); }
...
static void CG_RunCinematicFrame(int handle) { CIN_RunCinematic(handle); }
...
void CG_LoadHudMenu(void)
{
    ...
    cgDC.registerShaderNoMip = re->RegisterShaderNoMip;
    cgDC.setColor = re->SetColor;
    cgDC.drawHandlePic = CG_DrawPic;
    cgDC.drawStretchPic = re->DrawStretchPic;
    cgDC.registerModel = re->RegisterModel;
    cgDC.modelBounds = re->ModelBounds;
    cgDC.fillRect = &CG_FillRect;
    cgDC.drawRect = &CG_DrawRect;
    cgDC.drawSides = &CG_DrawSides;
    cgDC.drawTopBottom = &CG_DrawTopBottom;
    cgDC.clearScene = re->ClearScene;
    cgDC.addRefEntityToScene = re->AddRefEntityToScene;
    cgDC.renderScene = re->RenderScene;
    cgDC.registerFont = re->RegisterFont;
    ...
    // cgDC.executeText          = &Cbuf_ExecuteText;
...
void CG_AssetCache(void)
{
    ...
    cgDC.Assets.gradientBar = re->RegisterShaderNoMip(ASSET_GRADIENTBAR);
    cgDC.Assets.scrollBar = re->RegisterShaderNoMip(ASSET_SCROLLBAR);
    cgDC.Assets.scrollBarArrowDown = re->RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWDOWN);
    cgDC.Assets.scrollBarArrowUp = re->RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWUP);
    cgDC.Assets.scrollBarArrowLeft = re->RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWLEFT);
    cgDC.Assets.scrollBarArrowRight = re->RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWRIGHT);
    cgDC.Assets.scrollBarThumb = re->RegisterShaderNoMip(ASSET_SCROLL_THUMB);
    cgDC.Assets.sliderBar = re->RegisterShaderNoMip(ASSET_SLIDER_BAR);
    cgDC.Assets.sliderThumb = re->RegisterShaderNoMip(ASSET_SLIDER_THUMB);
    ...
    {
        cgDC.Assets.emoticons[i].shader = re->RegisterShaderNoMip(
...
void CG_Init(int serverMessageNum, int serverCommandSequence, int clientNum)
{
    ...
    cgs.media.whiteShader = re->RegisterShader("white");
    cgs.media.charsetShader = re->RegisterShader("gfx/2d/bigchars");
    cgs.media.outlineShader = re->RegisterShader("outline");
    ...
    // load the new map
	int	unused;
    CM_LoadMap(cgs.mapname, true, &unused);
...
static char *CG_VoIPString(void)
{
    ...
    Cvar_VariableStringBuffer("cl_voipSendTarget", voipSendTarget, sizeof(voipSendTarget));
