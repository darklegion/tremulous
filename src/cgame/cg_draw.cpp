...
static void CG_DrawPlayerBoosted(Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
    if (cg.snap->ps.stats[STAT_STATE] & SS_BOOSTED)
        re->SetColor(foreColor);
    else
        re->SetColor(backColor);
...
static void CG_DrawPlayerPoisonBarbs(Rectangle *rect, vec4_t color, qhandle_t shader)
{
    ...
    {
        re->DrawStretchPic(x, y, iconsize, iconsize, 0, 0, 1, 1, shader);
        if (vertical)
            y += diff;
        else
            x += diff;
...
static void CG_DrawPlayerChargeBar(Rectangle *rect, vec4_t ref_color, qhandle_t shader)
{
    ...
    if (rect->w >= rect->h)
    {
        ...
        // Draw the meter
        re->SetColor(color);
        re->DrawStretchPic(x, y, cap_size, height, 0, 0, 1, 1, shader);
        re->DrawStretchPic(x + width + cap_size, y, cap_size, height, 1, 0, 0, 1, shader);
        re->DrawStretchPic(x + cap_size, y, width, height, 1, 0, 1, 1, shader);
        re->SetColor(NULL);
    }
    ...
    else
    {
        ...
        // Draw the meter
        re->SetColor(color);
        re->DrawStretchPic(x, y - cap_size, width, cap_size, 0, 1, 1, 0, shader);
        re->DrawStretchPic(x, y - height - cap_size * 2, width, cap_size, 0, 0, 1, 1, shader);
        re->DrawStretchPic(x, y - height - cap_size, width, height, 0, 1, 1, 1, shader);
        re->SetColor(NULL);
    }
}
...
static void CG_DrawLevelShot(Rectangle *rect)
{
    ...
    levelshot = re->RegisterShaderNoMip(va("levelshots/%s.tga", s));

    if (!levelshot)
        levelshot = re->RegisterShaderNoMip("gfx/2d/load_screen");

    re->SetColor(NULL);
    CG_DrawPic(rect->x, rect->y, rect->w, rect->h, levelshot);

    // blend a detail texture over it
    detail = re->RegisterShader("gfx/misc/detail");
...
static void CG_DrawDisconnect(void)
{
    ...
    CG_DrawPic(x, y, 48, 48, re->RegisterShader("gfx/2d/net.tga"));
}
...
static void CG_DrawLagometer(Rectangle *rect, float text_x, float text_y, float scale, vec4_t textColor)
{
    ...
    for (a = 0; a < aw; a++)
    {
        ...
        if (v > 0)
        {
            ...
            re->DrawStretchPic(ax + aw - a, mid - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader);
        }
        else if (v < 0)
        {
            if (color != 2)
            ...
            re->DrawStretchPic(ax + aw - a, mid, 1, v, 0, 0, 0, 0, cgs.media.whiteShader);
        }
    }
    ...
    for (a = 0; a < aw; a++)
    {
        ...
        if (v > 0)
        {
            ...
            re->DrawStretchPic(ax + aw - a, ay + ah - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader);
        }
        else if (v < 0)
        {
            if (color != 4)
            ...
            re->DrawStretchPic(ax + aw - a, ay + ah - range, 1, range, 0, 0, 0, 0, cgs.media.whiteShader);
        }
    }
...
static bool CG_DrawScoreboard(void)
{
    ...
    if (menuScoreboard)
    {
        if (firstTime)
        {
            cg.spectatorTime = Sys_Milliseconds();
...
static void CG_DrawBinaryShadersFinalPhases(void)
{
    ...
    re->AddPolyToScene(cgs.media.binaryAlpha1Shader, 4, verts, 1);
    ...
    for (i = 0; i < cg.numBinaryShadersUsed; ++i)
    {
        ...
        if (cg.binaryShaderSettings[i].drawFrontline)
            re->AddPolyToScene(cgs.media.binaryShaders[i].f3, 4, verts, 1);
        if (cg.binaryShaderSettings[i].drawIntersection)
            re->AddPolyToScene(cgs.media.binaryShaders[i].b3, 4, verts, 1);
    }
