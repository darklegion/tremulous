...
void CG_FillRect(float x, float y, float width, float height, const float *color)
{
    ...
    re->DrawStretchPic(x, y, width, height, 0, 0, 0, 0, cgs.media.whiteShader);
...
void CG_DrawSides(float x, float y, float w, float h, float size)
{
    ...
    re->DrawStretchPic(x, y + sizeY, size, h - (sizeY * 2.0f), 0, 0, 0, 0, cgs.media.whiteShader);
    re->DrawStretchPic(x + w - size, y + sizeY, size, h - (sizeY * 2.0f), 0, 0, 0, 0, cgs.media.whiteShader);
}
...
void CG_DrawTopBottom(float x, float y, float w, float h, float size)
{
    ...
    re->DrawStretchPic(x, y, w, size, 0, 0, 0, 0, cgs.media.whiteShader);
    re->DrawStretchPic(x, y + h - size, w, size, 0, 0, 0, 0, cgs.media.whiteShader);
}
...
=================
*/
void CG_ClearClipRegion(void) { re->SetClipRegion(NULL); }
...
char *CG_KeyBinding(const char *bind)
{
    ...
    {
        Key_GetBindingBuf(i, bindbuff, sizeof(bindbuff));
        if (!Q_stricmp(bindbuff, bind))
        {
            Key_KeynumToStringBuf(i, key, sizeof(key));
            break;
        }
    }
