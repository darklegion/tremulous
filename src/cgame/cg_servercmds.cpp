    ...
    while (o && *o)
    {
        ...
        if (n && *n)
        {
            ...
            if (o)
            {
                ...
                timeOffset[o - t] = 0;
                o++;
                re->RemapShader(originalShader, newShader, timeOffset);
            }
        }
        else
            break;
...
static void CG_AnnounceAlienStageTransistion(stage_t from, stage_t to)
{
    ...
    S_StartLocalSound(cgs.media.alienStageTransition, CHAN_ANNOUNCER);
...
static void CG_AnnounceHumanStageTransistion(stage_t from, stage_t to)
{
    ...
    S_StartLocalSound(cgs.media.humanStageTransition, CHAN_ANNOUNCER);
...
static void CG_ConfigStringModified(void)
{
    ...
    else if (num >= CS_VOTE_TIME && num < CS_VOTE_TIME + NUM_TEAMS)
    {
        ...
        if (num - CS_VOTE_TIME == TEAM_NONE)
            Cvar_Set("ui_voteActive", cgs.voteTime[TEAM_NONE] ? "1" : "0");
        else if (num - CS_VOTE_TIME == TEAM_ALIENS)
            Cvar_Set("ui_alienTeamVoteActive", cgs.voteTime[TEAM_ALIENS] ? "1" : "0");
        else if (num - CS_VOTE_TIME == TEAM_HUMANS)
            Cvar_Set("ui_humanTeamVoteActive", cgs.voteTime[TEAM_HUMANS] ? "1" : "0");
    }
    ...
    else if (num >= CS_VOTE_NO && num < CS_VOTE_NO + NUM_TEAMS)
    {
        ...
        cgs.gameModels[num - CS_MODELS] = re->RegisterModel(str);
    else if (num >= CS_SHADERS && num < CS_SHADERS + MAX_GAME_SHADERS)
        cgs.gameShaders[num - CS_SHADERS] = re->RegisterShader(str);
...
static void CG_Say(int clientNum, saymode_t mode, const char *text)
{
    ...
    switch (mode)
    {
        ...
        case SAY_TPRIVMSG:
            ...
            {
                S_StartLocalSound(cgs.media.alienTalkSound, CHAN_LOCAL_SOUND);
                break;
            }
            else if (cg.snap->ps.stats[STAT_TEAM] == TEAM_HUMANS)
            {
                S_StartLocalSound(cgs.media.humanTalkSound, CHAN_LOCAL_SOUND);
                break;
            }
        default:
...
static void CG_ParseVoice(void)
{
    ...
    {
        case VOICE_CHAN_ALL:
            S_StartLocalSound(track->track, CHAN_VOICE);
            break;
        case VOICE_CHAN_TEAM:
            S_StartLocalSound(track->track, CHAN_VOICE);
            break;
        case VOICE_CHAN_LOCAL:
            S_StartSound(NULL, clientNum, CHAN_VOICE, track->track);
            break;
        default:
            break;
    }
...
static void CG_Chat_f(void)
{
    ...
    Cmd_ArgvBuffer(2, mode, sizeof(mode));
...
=================
*/
static void CG_ServerCloseMenus_f(void) { Cbuf_AddText("closemenus\n"); }
...
void CG_UnregisterCommands(void)
{
    ...
    while (registeredCmds[offset])
    {
        ...
        Cmd_RemoveCommand(registeredCmds + offset);
        offset += len + 1;
    }
...
static void CG_ServerCommand(void)
{
    ...
    cmd = CG_Argv(0);
    command = static_cast<consoleCommand_t*>(bsearch(cmd, svcommands, ARRAY_LEN(svcommands),
