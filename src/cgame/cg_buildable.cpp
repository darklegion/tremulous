...
void CG_AlienBuildableExplosion(vec3_t origin, vec3_t dir)
{
    ...
    S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.alienBuildableExplosion);
...
void CG_HumanBuildableExplosion(vec3_t origin, vec3_t dir)
{
    ...
    S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.humanBuildableExplosion);
...
void CG_InitBuildables(void)
{
    ...
    for (j = BANIM_NONE + 1; j < MAX_BUILDABLE_ANIMATIONS; j++)
    {
        ...
        defaultAlienSounds[j] = S_RegisterSound(filename, false);

        Com_sprintf(filename, sizeof(filename), "sound/buildables/human/%s", soundfile);
        defaultHumanSounds[j] = S_RegisterSound(filename, false);
    }
    ...
    for (i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++)
    {
        ...
        for (j = 0; j <= 3; j++)
        {
            ...
            if (strlen(modelFile) > 0)
                cg_buildables[i].models[j] = re->RegisterModel(modelFile);
        }
        ...
        for (j = BANIM_NONE + 1; j < MAX_BUILDABLE_ANIMATIONS; j++)
        {
            ...
            if (cg_buildables[i].sounds[j].enabled)
            {
                if (FS_FOpenFileByMode(filename, &f, FS_READ) > 0)
                {
                    ...
                    cg_buildables[i].sounds[j].sound = S_RegisterSound(filename, false);
                }
                else
                {
...
static void CG_RunBuildableLerpFrame(centity_t *cent)
{
    ...
    if (newAnimation != lf->animationNumber || !lf->animation)
    {
            ...
            cg_buildables[buildable].sounds[newAnimation].enabled)
        {
            ...
            S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO,
...
static void CG_PositionAndOrientateBuildable(const vec3_t angles, const vec3_t inOrigin, const vec3_t normal,
    const int skipNumber, vec3_t mins, vec3_t maxs, vec3_t outAxis[3], vec3_t outOrigin)
{
    vec3_t forward, end;
    trace_t tr;
    float fraction;
    ...
    while (1)
    {
        ...
        if (!Q_stricmp(token.string, "frameShader"))
        {
            if (PC_String_Parse(handle, &s))
                bs->frameShader = re->RegisterShader(s);
            continue;
        }
        else if (!Q_stricmp(token.string, "overlayShader"))
        {
            if (PC_String_Parse(handle, &s))
                bs->overlayShader = re->RegisterShader(s);
            continue;
        }
        else if (!Q_stricmp(token.string, "noPowerShader"))
        {
            if (PC_String_Parse(handle, &s))
                bs->noPowerShader = re->RegisterShader(s);
            continue;
        }
        else if (!Q_stricmp(token.string, "markedShader"))
        {
            if (PC_String_Parse(handle, &s))
                bs->markedShader = re->RegisterShader(s);
            continue;
        }
...
void CG_Buildable(centity_t *cent)
{
    ...
    if (!(es->eFlags & EF_B_SPAWNED))
    {
        ...
        S_AddLoopingSound(es->number, cent->lerpOrigin, vec3_origin, prebuildSound);
    }
    ...
    if (cg_buildables[es->modelindex].models[1])
    {
        ...
        re->AddRefEntityToScene(&turretBarrel);
    }
    ...
    if (cg_buildables[es->modelindex].models[2])
    {
        ...
        re->AddRefEntityToScene(&turretTop);
    }
    ...
    if (es->eFlags & EF_FIRING)
    {
            ...
            BG_Buildable(es->modelindex)->turretProjType == WP_TESLAGEN)
        {
            ...
            {
                re->AddLightToScene(cent->lerpOrigin, 300 + (rand() & 31),
        ...
        {
            S_AddLoopingSound(es->number, cent->lerpOrigin, vec3_origin, weapon->wim[WPM_PRIMARY].firingSound);
        }
        else if (weapon->readySound)
            S_AddLoopingSound(es->number, cent->lerpOrigin, vec3_origin, weapon->readySound);
    }
    ...
    if (health < cent->lastBuildableHealth && (es->eFlags & EF_B_SPAWNED))
    {
        if (cent->lastBuildableDamageSoundTime + BUILDABLE_SOUND_PERIOD < cg.time)
        {
            ...
            {
                int i = rand() % 4;
                S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.humanBuildableDamage[i]);
            }
            else if (team == TEAM_ALIENS)
                S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.alienBuildableDamage);
