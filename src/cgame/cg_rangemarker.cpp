...
void CG_UpdateBuildableRangeMarkerMask(void)
{
    ...
    empty:
        Cvar_Set("cg_buildableRangeMarkerMask", va("%i", brmMask));
...
void CG_DrawSphere(const vec3_t center, float radius, int customShader, const float *shaderRGBA)
{
    ...
    ref.reType = RT_MODEL;
    ref.hModel = cgs.media.sphereModel;
    ref.customShader = customShader;
    ref.renderfx = RF_NOSHADOW;
    if (shaderRGBA != NULL)
    {
        int i;
        for (i = 0; i < 4; ++i)
            ref.shaderRGBA[i] = 255 * shaderRGBA[i];
    }

    VectorCopy(center, ref.origin);

    radius *= 0.01f;
    VectorSet(ref.axis[0], radius, 0, 0);
    VectorSet(ref.axis[1], 0, radius, 0);
    VectorSet(ref.axis[2], 0, 0, radius);
    ...
    const vec3_t tip, const vec3_t rotation, float radius, bool a240, int customShader, const float *shaderRGBA)
{
    ...
    ref.reType = RT_MODEL;
    ref.hModel = a240 ? cgs.media.sphericalCone240Model : cgs.media.sphericalCone64Model;
    ref.customShader = customShader;
    ref.renderfx = RF_NOSHADOW;
    if (shaderRGBA != NULL)
    {
    ...
    VectorCopy(tip, ref.origin);

    radius *= 0.01f;
    AnglesToAxis(rotation, ref.axis);
    VectorScale(ref.axis[0], radius, ref.axis[0]);
    VectorScale(ref.axis[1], radius, ref.axis[1]);
    VectorScale(ref.axis[2], radius, ref.axis[2]);
    ref.nonNormalizedAxes = true;
