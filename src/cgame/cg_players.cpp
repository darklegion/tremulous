...
static bool CG_RegisterClientSkin(clientInfo_t *ci, const char *modelName, const char *skinName)
{
    ...
    else
    {
        ...
        ci->nonSegSkin = re->RegisterSkin(filename);
        if (!ci->nonSegSkin)
...
static bool CG_RegisterClientModelname(clientInfo_t *ci, const char *modelName, const char *skinName)
{
    ...
    else
    {
        ...
        ci->nonSegModel = re->RegisterModel(filename);
        if (!ci->nonSegModel)
        {
            Com_Printf("Failed to load model file %s\n", filename);
            return false;
...
static void CG_LoadClientInfo(clientInfo_t *ci)
{
    ...
    for (i = 0; i < MAX_CUSTOM_SOUNDS; i++)
    {
        ...
        if (!CG_FileExists(va("sound/player/%s/%s", dir, s + 1)))
        {
            ...
            {
                ci->sounds[i] = S_RegisterSound("sound/null.wav", false);
            }
            else
            {
                if (i == 9)  // gasp
    ...
    if (ci->footsteps == FOOTSTEP_CUSTOM)
    {
        for (i = 0; i < 4; i++)
        {
            ci->customFootsteps[i] = S_RegisterSound(va("sound/player/%s/step%d.wav", dir, i + 1), false);
            if (!ci->customFootsteps[i])
                ci->customFootsteps[i] = S_RegisterSound(va("sound/player/footsteps/step%d.wav", i + 1), false);

            ci->customMetalFootsteps[i] = S_RegisterSound(va("sound/player/%s/clank%d.wav", dir, i + 1), false);
            if (!ci->customMetalFootsteps[i])
                ci->customMetalFootsteps[i] =
                    S_RegisterSound(va("sound/player/footsteps/clank%d.wav", i + 1), false);
        }
    }
...
void CG_NewClientInfo(int clientNum)
{
    ...
    if (clientNum == cg.clientNum && ci->infoValid && ci->team != newInfo.team)
    {
        ...
        Cvar_VariableStringBuffer(va("cg_%sConfig", BG_TeamName(newInfo.team)), config, sizeof(config));
...
static void CG_PlayerUpgrades(centity_t *cent, refEntity_t *torso)
{
    ...
    if (held & (1 << UP_JETPACK))
    {
        ...
        re->AddRefEntityToScene(&jetpack);
    ...
    if (held & (1 << UP_BATTPACK))
    {
        ...
        re->AddRefEntityToScene(&battpack);
    }
...
static bool CG_PlayerShadow(centity_t *cent, float *shadowPlane, class_t klass)
{
    ...
    *shadowPlane = 0;
    ...
    CM_BoxTrace(&trace, cent->lerpOrigin, end, mins, maxs, 0, MASK_PLAYERSOLID, TT_AABB);
...
static void CG_PlayerSplash(centity_t *cent, class_t klass)
{
    ...
    CM_BoxTrace(&trace, start, end, NULL, NULL, 0,
            (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA), TT_AABB);
...
int CG_LightVerts(vec3_t normal, int numVerts, polyVert_t *verts)
{
    ...
    re->LightForPoint(verts[0].xyz, ambientLight, directedLight, lightDir);
...
int CG_LightFromDirection(vec3_t point, vec3_t direction)
{
    ...
    re->LightForPoint(point, ambientLight, directedLight, lightDir);
...
void CG_PlayerDisconnect(vec3_t org)
{
    ...
    S_StartSound(org, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.disconnectSound);
...
centity_t *CG_GetPlayerLocation(void)
{
    ...
    for (i = MAX_CLIENTS; i < MAX_GENTITIES; i++)
    {
        ...
        if (!re->inPVS(origin, eloc->lerpOrigin))
            continue;
