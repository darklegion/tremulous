            ...
            for (j = 0; j < bps->numEjectors; j++)
            {
                ...
                for (l = 0; l < bpe->numParticles; l++)
                {
                    ...
                    for (k = 0; k < bp->numFrames; k++)
                        bp->shaders[k] = re->RegisterShader(bp->shaderNames[k]);

                    for (k = 0; k < bp->numModels; k++)
                        bp->models[k] = re->RegisterModel(bp->modelNames[k]);

                    if (bp->bounceMarkName[0] != '\0')
                        bp->bounceMark = re->RegisterShader(bp->bounceMarkName);

                    if (bp->bounceSoundName[0] != '\0')
                        bp->bounceSound = S_RegisterSound(bp->bounceSoundName, false);
...
void CG_LoadParticleSystems(void)
{
    ...
    numFiles = FS_GetFileList("scripts", ".particle", fileList, MAX_PARTICLE_FILES * MAX_QPATH);
    filePtr = fileList;
...
#define MAX_ACC_RADIUS 1000.0f
    ...
    {
        S_StartSound(trace.endpos, ENTITYNUM_WORLD, CHAN_AUTO, bp->bounceSound);
        p->bounceSoundCount--;
    }
...
static void CG_RenderParticle(particle_t *p)
{
    ...
    ref.shaderTime = p->birthTime / 1000.0f;
    ...
    if (bp->numFrames)  // shader based
    {
        ...
        if (bp->realLight)
        {
            re->LightForPoint(p->origin, alight, dlight, lightdir);
            for (i = 0; i <= 2; i++)
                ref.shaderRGBA[i] = (byte)alight[i];
        }
        else
        {
            vec3_t colorRange;
        ...
        ref.shaderRGBA[3] = (byte)((float)0xFF * CG_LerpValues(p->alpha.initial, p->alpha.final,
                                                    CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->alpha.delay)));

        ref.radius = scale;

        ref.rotation = CG_LerpValues(
            p->rotation.initial, p->rotation.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->rotation.delay));

        // if the view would be "inside" the sprite, kill the sprite
        // so it doesn't add too much overdraw
        if (Distance(p->origin, cg.refdef.vieworg) < ref.radius && bp->overdrawProtection)
            return;
        ...
        if (bp->framerate == 0.0f)
        {
            ...
            ref.customShader = bp->shaders[index];
        }
        else
        {
            // looping animation
    ...
    else if (bp->numModels)  // model based
    {
        ...
        if (p->atRest)
            AxisCopy(p->lastAxis, ref.axis);
        else
        {
            // convert direction of travel into axis
            VectorNormalize2(p->velocity, ref.axis[0]);

            if (ref.axis[0][0] == 0.0f && ref.axis[0][1] == 0.0f)
                AxisCopy(axisDefault, ref.axis);
            else
            {
                ProjectPointOnPlane(ref.axis[2], up, ref.axis[0]);
                VectorNormalize(ref.axis[2]);
                CrossProduct(ref.axis[2], ref.axis[0], ref.axis[1]);
            }

            AxisCopy(ref.axis, p->lastAxis);
        }
        ...
        if (scale != 1.0f)
        {
            ...
            ref.nonNormalizedAxes = true;
        }
        else
            ref.nonNormalizedAxes = false;
        ...
        ref.oldframe = p->lf.oldFrame;
        ref.frame = p->lf.frame;
        ref.backlerp = p->lf.backlerp;
    }

    if (bps->thirdPersonOnly && CG_AttachmentCentNum(&ps->attachment) == cg.snap->ps.clientNum &&
        !cg.renderingThirdPerson)
        ref.renderfx |= RF_THIRD_PERSON;

    if (bp->dynamicLight && !(ref.renderfx & RF_THIRD_PERSON))
    {
        re->AddLightToScene(p->origin,
    ...
    VectorCopy(p->origin, ref.origin);
