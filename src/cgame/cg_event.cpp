...
void CG_PainEvent(centity_t *cent, int health)
{
    ...
    S_StartSound(NULL, cent->currentState.number, CHAN_VOICE, CG_CustomSound(cent->currentState.number, snd));
...
void CG_EntityEvent(centity_t *cent, vec3_t position)
{
    ...
    switch (event)
    {
        ...
        case EV_FOOTSTEP:
            if (cg_footsteps.integer && ci->footsteps != FOOTSTEP_NONE)
            {
                if (ci->footsteps == FOOTSTEP_CUSTOM)
                    S_StartSound(NULL, es->number, CHAN_BODY, ci->customFootsteps[rand() & 3]);
                else
        ...
        case EV_FOOTSTEP_METAL:
            ...
            {
                if (ci->footsteps == FOOTSTEP_CUSTOM)
                    S_StartSound(NULL, es->number, CHAN_BODY, ci->customMetalFootsteps[rand() & 3]);
                else
        ...
        case EV_FOOTSTEP_SQUELCH:
            if (cg_footsteps.integer && ci->footsteps != FOOTSTEP_NONE)
            {
                S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.footsteps[FOOTSTEP_FLESH][rand() & 3]);
            }
            break;
        ...
        case EV_FALL_SHORT:
            S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.landSound);
        ...
        case EV_FALL_MEDIUM:
            ...
            S_StartSound(NULL, es->number, CHAN_VOICE, CG_CustomSound(es->number, "*pain100_1.wav"));
        ...
        case EV_FALL_FAR:
            S_StartSound(NULL, es->number, CHAN_AUTO, CG_CustomSound(es->number, "*fall1.wav"));
        ...
        case EV_FALLING:
            S_StartSound(NULL, es->number, CHAN_AUTO, CG_CustomSound(es->number, "*falling1.wav"));
            break;
        ...
        case EV_JUMP:
            S_StartSound(NULL, es->number, CHAN_VOICE, CG_CustomSound(es->number, "*jump1.wav"));
        ...
        case EV_LEV1_GRAB:
            S_StartSound(NULL, es->number, CHAN_VOICE, cgs.media.alienL1Grab);
            break;

        case EV_LEV4_TRAMPLE_PREPARE:
            S_StartSound(NULL, es->number, CHAN_VOICE, cgs.media.alienL4ChargePrepare);
            break;

        case EV_LEV4_TRAMPLE_START:
            // FIXME: stop cgs.media.alienL4ChargePrepare playing here
            S_StartSound(NULL, es->number, CHAN_VOICE, cgs.media.alienL4ChargeStart);
            break;

        case EV_TAUNT:
            if (!cg_noTaunt.integer)
                S_StartSound(NULL, es->number, CHAN_VOICE, CG_CustomSound(es->number, "*taunt.wav"));
            break;
        ...
        //
        case EV_NOAMMO:
            S_StartSound(NULL, es->number, CHAN_WEAPON, cgs.media.weaponEmptyClick);
            break;

        case EV_CHANGE_WEAPON:
            S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.selectSound);
            break;
        ...
        case EV_RPTUSE_SOUND:
            S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.repeaterUseSound);
            break;

        case EV_GRENADE_BOUNCE:
            if (rand() & 1)
                S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.hardBounceSound1);
            else
                S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.hardBounceSound2);
            break;
        ...
        case EV_BUILD_DELAY:
            if (clientNum == cg.predictedPlayerState.clientNum)
            {
                S_StartLocalSound(cgs.media.buildableRepairedSound, CHAN_LOCAL_SOUND);
                cg.lastBuildAttempt = cg.time;
            }
            break;

        case EV_BUILD_REPAIR:
            S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.buildableRepairSound);
            break;

        case EV_BUILD_REPAIRED:
            S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.buildableRepairedSound);
            break;

        case EV_OVERMIND_ATTACK:
            if (cg.predictedPlayerState.stats[STAT_TEAM] == TEAM_ALIENS)
            {
                S_StartLocalSound(cgs.media.alienOvermindAttack, CHAN_ANNOUNCER);
        ...
        case EV_OVERMIND_DYING:
            if (cg.predictedPlayerState.stats[STAT_TEAM] == TEAM_ALIENS)
            {
                S_StartLocalSound(cgs.media.alienOvermindDying, CHAN_ANNOUNCER);
        ...
        case EV_DCC_ATTACK:
            ...
            {
                // S_StartLocalSound( cgs.media.humanDCCAttack, CHAN_ANNOUNCER );
                CG_CenterPrint("Our base is under attack!", 200, GIANTCHAR_WIDTH * 4);
            }
            break;

        case EV_MGTURRET_SPINUP:
            S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.turretSpinupSound);
            break;

        case EV_OVERMIND_SPAWNS:
            if (cg.predictedPlayerState.stats[STAT_TEAM] == TEAM_ALIENS)
            {
                S_StartLocalSound(cgs.media.alienOvermindSpawns, CHAN_ANNOUNCER);
                CG_CenterPrint("The Overmind needs spawns!", 200, GIANTCHAR_WIDTH * 4);
            }
            break;

        case EV_ALIEN_EVOLVE:
            S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.alienEvolveSound);
            {
        ...
        case EV_MEDKIT_USED:
            S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.medkitUseSound);
            break;
